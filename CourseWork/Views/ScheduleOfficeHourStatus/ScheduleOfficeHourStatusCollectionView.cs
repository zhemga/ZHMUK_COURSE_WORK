...
namespace CourseWork.Views.ScheduleOfficeHourStatusCollectionView{
    public partial class ScheduleOfficeHourStatusCollectionView : XtraUserControl {
        public ScheduleOfficeHourStatusCollectionView() {
            InitializeComponent();
			if(!mvvmContext.IsDesignMode)
                InitBindings();
        }
        void InitBindings() {
            var fluentAPI = mvvmContext.OfType<CourseWork.ViewModels.ScheduleOfficeHourStatusCollectionViewModel>();
            ...
            fluentAPI.WithEvent<GridView, FocusedRowObjectChangedEventArgs>(gridView, "FocusedRowObjectChanged")
                .SetBinding(x => x.SelectedEntity,
                    args => args.Row as CourseWork.Model.ScheduleOfficeHourStatus,
