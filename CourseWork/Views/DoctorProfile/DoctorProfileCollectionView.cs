...
namespace CourseWork.Views.DoctorProfileCollectionView{
    public partial class DoctorProfileCollectionView : XtraUserControl {
        public DoctorProfileCollectionView() {
            InitializeComponent();
			if(!mvvmContext.IsDesignMode)
                InitBindings();
        }
        void InitBindings() {
            var fluentAPI = mvvmContext.OfType<CourseWork.ViewModels.DoctorProfileCollectionViewModel>();
            ...
            fluentAPI.WithEvent<GridView, FocusedRowObjectChangedEventArgs>(gridView, "FocusedRowObjectChanged")
                .SetBinding(x => x.SelectedEntity,
                    args => args.Row as CourseWork.Model.DoctorProfile,
