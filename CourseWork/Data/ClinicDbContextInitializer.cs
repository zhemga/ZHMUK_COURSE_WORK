...
new Doctor {
    ...
    new Patient {
        ...
        IsMaleOrElseFemaleSex = true
            ...
            DateTime minDateTime = new DateTime(2000, 01, 01, 0, 0, 0);
            const int minutesThatTakeOfficeHours = 45;
            foreach (var doctor in doctors)
            {
                for (int dayOfWeekNumber = 1; dayOfWeekNumber < 8; ++dayOfWeekNumber)
                {
                    for (int hoursToAdd = 8; hoursToAdd < 20; ++hoursToAdd)
                    {
                        var scheduleOfficeHoursStartTime = minDateTime.AddHours(hoursToAdd);
                        scheduleOfficeHours.Add(new ScheduleOfficeHour
                        {
                            Id = Guid.NewGuid(),
                            Description = "Перевірка здоров'я",
                            DayOfWeekNumber = dayOfWeekNumber,
                            DoctorId = doctor.Id,
                            StartTime = scheduleOfficeHoursStartTime,
                            EndTime = scheduleOfficeHoursStartTime.AddMinutes(minutesThatTakeOfficeHours),
                            ScheduleOfficeHourStatusStatusName = scheduleOfficeHourStatuses[0].Name
                        });
                    }
                }
            }

            for (int i = 0; i < scheduleOfficeHours.Count; i += 5)
            {
                scheduleOfficeHours[i].PatientId = patients[random.Next(0, patients.Count)].Id;

                if(scheduleOfficeHours[i].DayOfWeekNumber < 3)
                {
                    scheduleOfficeHours[i].ScheduleOfficeHourStatus = scheduleOfficeHourStatuses[random.Next(1, scheduleOfficeHourStatuses.Count)];
                }
            }
